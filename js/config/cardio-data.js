...
const CARDIO_TYPES = {
  treadmill: {
    ...
    requiredLabel: "Distância • inclinação", hint: "Ao finalizar, informe a distância e, se quiser, a inclinação média. A velocidade média é calculada automaticamente usando distância e tempo.", fields: [
      { key: "distance", label: "Distância", unit: "km", type: "number", min: 0, max: 250, step: 0.01, required: true },
      { key: "incline", label: "Inclinação média", unit: "%", type: "number", min: 0, max: 40, step: 0.5, required: false }
    ],
    limits: { maxSpeedKmh: 45 }
  },
  outdoor_run: {
  ...
  stationary_bike: {
    ...
    requiredLabel: "Distância • resistência • RPM", hint: "Resistência e duração geram Constituição. Cadência alta e evolução de performance podem gerar Agilidade.", fields: [
      { key: "distance", label: "Distância", unit: "km", type: "number", min: 0, max: 500, step: 0.1, required: true },
      { key: "resistance", label: "Resistência média", unit: "nível", type: "number", min: 0, max: 100, step: 1, required: false },
      { key: "rpm", label: "Cadência média", unit: "RPM", type: "number", min: 0, max: 250, step: 1, required: false }
    ],
    limits: { maxSpeedKmh: 120, maxRpm: 250 }
  },
  outdoor_bike: {
  ...
  elliptical: {
    ...
    requiredLabel: "Distância • resistência", hint: "Use a distância e o nível médio mostrados no painel da máquina.", fields: [
      { key: "distance", label: "Distância", unit: "km", type: "number", min: 0, max: 250, step: 0.1, required: false },
      { key: "resistance", label: "Resistência média", unit: "nível", type: "number", min: 0, max: 100, step: 1, required: false }
    ],
    limits: { maxSpeedKmh: 45 }
  },
  stair_climber: {
    ...
    requiredLabel: "Andares • nível", hint: "Endurance gera Constituição; uma sessão consistente também pode gerar uma parcela menor de Força pelas pernas.", fields: [
      { key: "floors", label: "Andares", unit: "andares", type: "number", min: 0, max: 5000, step: 1, required: false },
      { key: "resistance", label: "Nível médio", unit: "nível", type: "number", min: 0, max: 100, step: 1, required: false }
    ],
    limits: { maxFloorsPerMinute: 20 }
  },
  rowing: {
    ...
    requiredLabel: "Distância • ritmo /500 m", hint: "Endurance gera Constituição; ritmo e esforço do remo podem gerar uma parcela menor de Força.", fields: [
      { key: "distanceMeters", label: "Distância", unit: "m", type: "number", min: 0, max: 100000, step: 10, required: true }
    ],
    limits: { maxSpeedKmh: 40 }
  },
  jump_rope: {
    ...
    requiredLabel: "Saltos", hint: "Cadência e coordenação geram Agilidade; sessões sustentadas também podem gerar Constituição.", fields: [
      { key: "jumps", label: "Saltos", unit: "saltos", type: "number", min: 0, max: 100000, step: 1, required: true }
    ],
    limits: { maxJumpsPerMinute: 400 }
  },
  swimming: {
    ...
    requiredLabel: "Distância • piscina", hint: "Endurance gera Constituição; ritmo e eficiência de deslocamento podem gerar Agilidade.", fields: [
      { key: "distanceMeters", label: "Distância", unit: "m", type: "number", min: 0, max: 100000, step: 25, required: true },
      { key: "poolLength", label: "Comprimento da piscina", unit: "m", type: "number", min: 0, max: 100, step: 5, required: false }
    ],
    limits: { maxSpeedKmh: 10 }
  }
};
